...
//        print(String(data: data, encoding: .utf8)!)
    ...
    func deleteComputerEA(server: String,resourceType: ResourceType, itemID: String, authToken: String) async throws {
        let resourcePath = getURLFormat(data: (ResourceType.computerExtensionAttribute))
    ...
    func batchDeleteComputerEA(selection:  Set<ComputerExtensionAttribute>, server: String, authToken: String, resourceType: ResourceType) async throws {
        ...
        for eachItem in selection {
            ...
            do {
                try await self.deleteComputerEA(server: server, resourceType: ResourceType.computerExtensionAttribute, itemID: jamfID, authToken: authToken )
            } catch {
    ...
    func updateComputerEAValueMultipleComputers(server: String, authToken: String, computerIds: Set<Int>, extAttName: String, updateValue: String) async throws {
        let name = extAttName.trimmingCharacters(in: .whitespacesAndNewlines)
        let value = updateValue.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !name.isEmpty else {
            print("updateComputerEAValueMultipleComputers: name empty, aborting")
            return
        }
        ...
        for computerId in computerIds {
            ...
            guard let http = response as? HTTPURLResponse, (200...299).contains(http.statusCode) else {
                ...
                self.hasError = true
                self.currentResponseCode = String(describing: statusCode)
                print("updateComputerEAValueMultipleComputers failed for computer \(computerId) with status: \(statusCode)")
                throw JamfAPIError.http(statusCode)
            }
